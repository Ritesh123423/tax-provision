...
/* =====================================================
   ...
   SheetJS loaded from CDN on demand
===================================================== */
const Excel = (() => {
  ...
  async function exportWorkpaper() {
    ...
    if (!window.XLSX) {
      try {
        ...
        UI.toast('Could not load export library. Check internet.', 'err'); return;
      }
    }
    ...
    WB.Props = { Title: 'Tax Provision Workpaper', Author: 'K G Somani & Co LLP', CreatedDate: new Date() };
...
/* =====================================================
   UI.JS - Navigation, Toast, Controls
===================================================== */
const UI = (() => {
  ...
  function toast(msg, type = 'info', dur = 3000) {
    ...
    wrap.appendChild(t);
    setTimeout(() => { t.style.opacity = '0'; t.style.transform = 'translateY(6px)'; t.style.transition = 'all .2s'; setTimeout(() => t.remove(), 200); }, dur);
  }

  function compute() {
    Compute.run(State.data);
    toast('Computed successfully', 'ok', 2000);
  }
  ...
  function updateClient() {
    ...
    const cn = $('sb-client'); if (cn && n) cn.textContent = n;
    const cf = $('sb-fy');     if (cf && fy) cf.textContent = fy;
  ...
  function init() {
    ...
    const fi  = $('ci-fy');   if (fi && !fi.value) fi.value = yr + '-' + String(yr + 1).slice(-2);
    ...
    ['ci-name','ci-fy'].forEach(id => {
      const e = $(id); if (e) e.addEventListener('input', updateClient);
    });
  ...
  return { nav, toast, compute, print_, updateClient, reset, init };
})();
